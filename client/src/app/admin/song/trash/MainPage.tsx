...
export const MainPage = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/admin/song/trash/list${query}`, {
      credentials: "include"
    })
      .then(res => res.json())
      .then((data) => {
  ...
  const handleApplyMulti = () => {
    if (idList && idList.length && applyMultiStatus) {
      ...
      const promise = fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/admin/song/trash/apply-multi`, {
        method: "PATCH",
        headers: {
  ...
  const handleRecovery = (id) => {
    ...
    const promise = fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/admin/song/trash/recovery`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json"
      },
