...
export const SongCreateForm = () => {
  ...
  useEffect(() => {
    if (singerList.length === 0 && categoryTree.length === 0) return;

    const validation = new JustValidate('#song-create-form');
  ...
  return (
    <>  
      <Toaster/>
      <form id="song-create-form" className="bg-white border-[0.3px] border-[#B9B9B9] rounded-[14px] p-[20px] sm:p-[50px]" onSubmit={handleSubmit}>
            ...
            <div className="h-[150px] py-[18px] px-[23px] bg-[#F5F6FA] rounded-[4px] border-[0.6px] border-[#D5D5D5] overflow-y-scroll flex flex-col gap-[10px]">
              {singerList && singerList.length > 0 && singerList.map((item, index) => (
                <div className="flex items-center gap-[10px]" key={index}>
                  <input name="singer" type="checkbox" className="w-[18px] h-[18px]" value={item.id} />
